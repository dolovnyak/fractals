...
/*   Updated: 2019/05/07 10:58:58 by sbecker          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	set_args_and_run(t_mandel2d *conf, cl_mem *mem_img)
{
	double	step_x;
	double	step_y;
	size_t	global_size;
	...
	kernel = conf->cl->kernel_mandel2d;
	...
	err |= clSetKernelArg(kernel, 7, sizeof(int), &conf->color_mult);
	if (err != 0)
...
static int	refresh(t_mandel2d *conf)
{
	int		err;
	cl_mem	mem_img;
...
void		mandelbrot_2d(t_cl *cl, t_mlx *mlx)
{
	...
	mlx_hook(conf.mlx->win, 2, 0, &mandel2d_keyboard_event, &conf);
	mlx_hook(conf.mlx->win, 4, 0, &mandel2d_mouse_press, &conf);
