...
/*   Updated: 2019/05/02 04:20:50 by sbecker          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_args_and_run(t_conf *conf, cl_mem *mem_arr_structs, int *z, size_t global_size)
{
	int	err;

	err = clSetKernelArg(conf->cl.kernel_mandel, 0, sizeof(cl_mem), mem_arr_structs);
	err = clSetKernelArg(conf->cl.kernel_mandel, 1, sizeof(int), z);
	err = clSetKernelArg(conf->cl.kernel_mandel, 2, sizeof(t_fractal), &conf->fractal);
	if (err != 0)
		printf("set kernel args - error\n");
	err = clEnqueueNDRangeKernel(conf->cl.queue, conf->cl.kernel_mandel, 1, NULL,
...
t_fractal_point	**get_fractal(t_conf *conf, size_t global_size)
{
	...
	int				err;
	cl_int			tmp_objects_num;

	get_steps(conf);
	all_points = (t_fractal_point**)malloc(sizeof(t_fractal_point*) * conf->fractal.z_len);
	i = -1;
	while (++i < conf->fractal.z_len)
	{
		xy_points = (t_fractal_point*)malloc(sizeof(t_fractal_point) * global_size);
