...
/*   Updated: 2019/05/16 22:58:51 by sbecker          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	set_args_and_run(t_fractal2d *conf, cl_mem *mem_img)
{
	...
	kernel = conf->cl->kernel_julia2d_pow5;
	...
	err |= clSetKernelArg(kernel, 7, sizeof(double), &conf->const_real);
	err |= clSetKernelArg(kernel, 8, sizeof(double), &conf->const_imagin);
	err |= clSetKernelArg(kernel, 9, sizeof(double), &conf->radius);
	err |= clSetKernelArg(kernel, 10, sizeof(int), &conf->color_mult);
...
static int	julia2d_refresh(t_fractal2d *conf)
{
	int		err;
	cl_mem	mem_img;
...
void		julia_2d_pow5(t_cl *cl)
{
	...
	conf.cl = cl;
	conf.mlx = &mlx;
	julia2d_initialization(&conf);
	mlx_hook(conf.mlx->win, 2, 0, &julia2d_key_press, &conf);
	mlx_hook(conf.mlx->win, 4, 0, &common2d_mouse_press, &conf);
	mlx_hook(conf.mlx->win, 6, 0, &julia2d_mouse_move, &conf);
	mlx_hook(conf.mlx->win, 17, 0, &exit_event, conf.cl);
	mlx_loop_hook(conf.mlx->mlx, &julia2d_refresh, &conf);
