...
/*   Updated: 2019/05/16 23:11:42 by sbecker          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	get_root_and_set_args(t_fractal2d *conf, int *err)
{
	...
	kernel = conf->cl->kernel_fractal_dick;
...
static int	dick_refresh(t_fractal2d *conf)
{
	int		err;
	cl_mem	mem_img;
...
void		fractal_dick(t_cl *cl)
{
	...
	conf.cl = cl;
	conf.mlx = &mlx;
	fractal_dick_initialization(&conf);
	...
	mlx_loop_hook(conf.mlx->mlx, &dick_refresh, &conf);
