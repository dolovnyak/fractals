...
static int	check_convergence_dick(t_complex z1, t_complex z2, double margin_error)
{
	t_complex	z;
...
__kernel void fractal_dick(__global int* img, double step_x, double step_y, int depth,
		...
		t_complex root2, t_complex root3, t_complex root4, t_complex root5, int color_mult)
{
	...
	while (++i < depth)
	{
		z = cx_minus(z, cx_dev(cx_minus_real(z, 1), cx_mult_real(z, 5)));
